...
const getProductBySlug = async (req, res) => {
    ...
    const product = await Product_1.ProductModel.findOne({ baseSlug: slug }).lean();
    if (!product)
        return res.status(404).json({ error: "Product not found" });
    const selectedVariantId = variantId || (product.variants[0]?._id?.toString() ?? null);
    const plans = selectedVariantId
        ? await EMIPlan_1.EMIPlanModel.find({
