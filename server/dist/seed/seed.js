...
const EMIPlan_1 = require("../models/EMIPlan");
...
(async () => {
    ...
    await EMIPlan_1.EMIPlanModel.deleteMany({});
    await Product_1.ProductModel.deleteMany({});
    for (const p of productsData) {
        const baseSlug = (0, slug_1.toSlug)(p.name);
        const product = await Product_1.ProductModel.create({ ...p, baseSlug });
        const plans = mkPlans(product);
        await EMIPlan_1.EMIPlanModel.insertMany(plans);
    }
